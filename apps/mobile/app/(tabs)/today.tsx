...
export default function TodayScreen() {
  ...
  const noteSections = [...(store.noteGroups ?? []).sort((a, b) => a.order - b.order).map((group) => ({ name: group.name, notes: store.notes.filter((note) => note.groupId === group.id) })), { name: "Unsorted", notes: store.notes.filter((note) => !note.groupId) }].filter((section) => section.notes.length > 0);
    ...
    <ScrollView style={s.screen} contentContainerStyle={s.content}>
      ...
      >
        {noteSections.length === 0 ? (
          ...
          <>
            {noteSections.map((section) => <Accordion key={section.name} label={section.name} badge={`${section.notes.length}`} nested>{section.notes.map((note) => <Accordion key={note.id} label={note.heading} nested>{note.description ? <Text selectable style={s.noteDescExpanded}>{note.description}</Text> : <Text style={s.muted}>No description.</Text>}</Accordion>)}</Accordion>)}
          </>
        )}
      </Accordion>
